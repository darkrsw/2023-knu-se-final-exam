...
class _Pickler:
    ...
    def save_global(self, obj, name=None, pack=struct.pack):
        ...
        if self.proto >= 3:
            ...
            if self.fix_imports:
                if (module, name) in _codes.REVERSE_NAME_MAPPING:
                    module, name = _codes.REVERSE_NAME_MAPPING[(module, name)]
                if module in _codes.REVERSE_IMPORT_MAPPING:
                    module = _codes.REVERSE_IMPORT_MAPPING[module]
            try:
...
class _Unpickler:
    ...
    def find_class(self, module, name):
        ...
        if self.proto < 3 and self.fix_imports:
            if (module, name) in _codes.NAME_MAPPING:
                module, name = _codes.NAME_MAPPING[(module, name)]
            if module in _codes.IMPORT_MAPPING:
                module = _codes.IMPORT_MAPPING[module]
        __import__(module, level=0)
        mod = sys.modules[module]
        klass = getattr(mod, name)
        return klass
